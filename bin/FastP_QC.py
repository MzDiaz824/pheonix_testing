...
##Written by Rich Stanton (njr5@cdc.gov), Jill Hagey (qpk9@cdc.gov) and Nick Vlachos (nvx4@cdc.gov)
...
def FastP_QC_raw(input_json, output_file, name):
...
def FastP_QC_trimmed(input_trimmed_json, input_singles_json, output_file, name):
    ...
    Q20_Total_trimmed = paired_total_info['q20_bases']
    Q30_Total_trimmed = paired_total_info['q30_bases']
    Trimmed_Sequenced_bp = str(paired_total_info['total_bases'])
    Trimmed_Sequenced_reads = str(paired_total_info['total_reads'])
    Info1 = data['read1_before_filtering']  # this was read1_after_filtering and threw and error as that didn't exist
    raw_R1_reads = str(Info1['total_reads'])
    raw_R1_bases = str(Info1['total_bases'])
    Q20_R1_bp = str(Info1['q20_bases'])
    Q20_R1_bp = str(Info1['q20_bases'])
    ...
    Q20_Total_singles = singles_total_info['q20_bases']
    Q30_Total_singles = singles_total_info['q30_bases']
    ...
    unpaired_R1_reads = str(Info1['total_reads'])
    unpaired_R1_bases = str(Info1['total_bases'])
    Q20_R1_bp = str(Info1['q20_bases'])
    Q20_R1_percent = str(Decimal(Info1['q20_bases']) / Decimal(Info1['total_bases']))
    Q30_R1_bp = str(Info1['q30_bases'])
    Q30_R1_percent = str(Decimal(Info1['q30_bases']) / Decimal(Info1['total_bases']))
    ...
    Total_Sequenced_bp = Trimmed_Sequenced_bp + Singles_Sequenced_bp
    Total_Sequenced_reads = Trimmed_Sequenced_reads + Singles_Sequenced_reads

    Q20_Total = str(Q20_Total_trimmed + Q20_Total_singles) # Nick double check this
    Q30_Total = str(Q30_Total_trimmed + Q30_Total_singles) # Nick double check this
    Q20_unpaired_bp = Q20_R1_bp + Q20_R2_bp
    Q20_unpaired_percent = str(Decimal(Q20_unpaired_bp) / Decimal(Total_Sequenced_bp))
    Q30_unpaired_bp = Q30_R1_bp + Q30_R2_bp
    Q30_unpaired_percent = str(Decimal(Q30_unpaired_bp) / Decimal(Total_Sequenced_bp))
...
#def FastP_QC_All(input_folder, name):
#    input_paired_json=input_folder+"/"+fastp_trimd+"/"+name+".fastp.json"
#    input_singles_json=input_folder+"/"+fastp_singles+"/"+name+".fastp.json"
#    """Makes a pre- and post-filtering output of trimmed info"""
#    FastP_QC_raw(input_paired_json, input_folder+"/"+fastp_trimd+"/"+name+"_raw_read_counts.txt")
    #FastP_QC(input_reads_json, name + '_trimmed_counts.txt', 'after', name)
#    FastP_QC_trimmed(input_paired_json, input_folder+"/"+fastp_trimd+"/"+name+"_trimmed_read_counts.txt")

def FastP_QC_All_JH(input_paired_json, input_singles_json, name):
    """Makes a pre- and post-filtering output of trimmed info"""
    """FastP_QC_raw(input_json, output_file, name)"""
    raw_output= name + "_raw_read_counts.txt"
    FastP_QC_raw(input_paired_json, raw_output, name)
    """FastP_QC_trimmed(input_trimmed_json, input_singles_json, output_file, name)"""
    trimmed_output= name + "_trimmed_read_counts.txt"
    FastP_QC_trimmed(input_paired_json, input_singles_json, trimmed_output, name)

def main():
    # Inputs are location based, #1-isolate_folder_path #2-isolate_name
    #FastP_QC_All_JH(sys.argv[0], sys.argv[1], sys.argv[2])
    args = parse_cmdline()
    # Inputs are location based, #1-isolate_folder_path #2-isolate_name
    FastP_QC_All_JH(args.trimmed_json, args.single_json, args.name)
