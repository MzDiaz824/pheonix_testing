	...
	if checking:
		...
		if mlstype != new_types or "-" in mlstype or "AU" in mlstype or "SUB" in mlstype or "NAF" in mlstype:
			for i in range(0, len(new_types)):
				if "-" in str(new_types[i]):
					new_types[i] = 12000
				elif "AU" in str(new_types[i]):
					new_types[i] = 13000
				elif "A-SUB" in str(new_types[i]):
					new_types[i] = 14000
				elif "P-SUB" in str(new_types[i]):
					new_types[i] = 15000
				elif "NAF" in str(new_types[i]):
					new_types[i] = 16000
				else:
					new_types[i] = int(new_types[i])
			new_types.sort()
			for i in range(0, len(new_types)):
				if new_types[i] == 12000:
					new_types[i] = '-'
				elif new_types[i] == 13000:
					new_types[i] = 'AU'
				elif new_types[i] == 14000:
					new_types[i] = 'A-SUB'
				elif new_types[i] == 15000:
					new_types[i] = 'P-SUB'
				elif new_types[i] == 16000:
					new_types[i] = 'NAF'
				else:
...
def get_type(list_of_profiles, list_of_allele_names, DB_file, source_filetype):
	...
	full_db_path="/scicomp/groups/OID/NCEZID/DHQP/CEMB/databases/tools/mlst/db/pubmlst/"+DB_file+"/"+DB_file+".txt"
	...
	for i in range(0, len(types)):
		#print("type_check#",i,"of", len(types), ":", types[i])
		if types[i] == -1:
			passed="true"
...
def find_DB_taxonomy(genus, species):
	...
	species_exists = os.path.exists('/scicomp/groups/OID/NCEZID/DHQP/CEMB/databases/pubmlst/'+db_test_species)
	genus_exists = os.path.exists('/scicomp/groups/OID/NCEZID/DHQP/CEMB/databases/pubmlst/'+genus.lower())
	species_path = Path('/scicomp/groups/OID/NCEZID/DHQP/CEMB/databases/pubmlst/'+db_test_species)
	genus_path = Path('/scicomp/groups/OID/NCEZID/DHQP/CEMB/databases/pubmlst/'+db_test_species)
