...
## :warning: Please read this documentation on the nf-core website: [https://nf-co.re/qtest/usage](https://nf-co.re/qtest/usage)
...
```console
nextflow run nf-core/qtest --input samplesheet.csv --genome GRCh37 -profile docker
```
...
First, go to the [nf-core/qtest releases page](https://github.com/nf-core/qtest/releases) and find the latest version number - numeric only (eg. `1.3.1`). Then specify this when running the pipeline with `-r` (one hyphen) - eg. `-r 1.3.1`.
